...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	private boolean allowCircularReferences = true;	//spring中默认支持循环引用
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (mbd.isSingleton()) {
			/**
			 * 如果bean指定需要通过factoryMethod来创建则会在这里被创建
			 */
			instanceWrapper = this.factoryBeanInstanceCache.remove(beanName);
		}
		if (instanceWrapper == null) {
			//通过反射创建临时对象（不是bean），可以理解为new了一个对象。
			instanceWrapper = createBeanInstance(beanName, mbd, args);
		}
		//new出来的临时对象会被存储到该对象（instanceWrapper）的wrappedInstance属性中，所以此处需要getWrappedInstance()
		...
		boolean earlySingletonExposure = (mbd.isSingleton() //单例（可以看出spring默认prototype的类不支持循环引用，但可以通过拓展实现）
				&& this.allowCircularReferences 	//允许循环引用
				&& isSingletonCurrentlyInCreation(beanName));	//正在被创建
		if (earlySingletonExposure) {
			if (logger.isTraceEnabled()) {
				logger.trace("Eagerly caching bean '" + beanName +
						"' to allow for resolving potential circular references");
			}
			//将该临时对象的信息（singletonFactory，不是直接放对象进去）放到singletonFactories的Map中
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (hasInstAwareBpps) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					//通过postProcessProperties()方法完成属性填充
